...
contract Farming is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 onlyVIP;          // Only VIP allowed.
    }
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint256 _isVIP, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        DPC.mint(airdropAddress, DPCReward.mul(5).div(65));
        DPC.mint(teamAddress, DPCReward.mul(15).div(65));
        DPC.mint(seedAddress, DPCReward.mul(5).div(65));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (pool.onlyVIP > 0) {
            uint256 quota = vips[msg.sender];
            require(quota > 0, "only VIP allowed");
            uint256 calculatedQuota = getDPCAmount(_pid, _amount);
            uint256 allowedQuota = user.amount.add(calculatedQuota);
            require(allowedQuota <= quota, "quota exceeded");
        }
    ...
    //Add vip
    function addVIP(address _vip, uint256 _quota) public onlyOwner{
        vips[_vip] = _quota;
    }
}
